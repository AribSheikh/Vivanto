...
// Public slot list: slot number, status, and (per explicit site-owner
// instruction) name + CNIC for allocated slots. The site owner controls
// what value goes into the CNIC field via /admin and has confirmed real
// CNICs won't be entered there -- this function just displays whatever
// was entered, it doesn't validate or guarantee it's a real ID number.
export async function getPublicSlotStatuses() {
  if (sql) {
    await ensureSeededDb();
    const rows = await sql`SELECT slot_number, status, name, cnic FROM slots ORDER BY slot_number`;
    return rows.map((r) => ({ slotNumber: r.slot_number, status: r.status, name: r.name, cnic: r.cnic }));
  }
  return fileReadSlots().map((s) => ({ slotNumber: s.slotNumber, status: s.status, name: s.name, cnic: s.cnic }));
}
